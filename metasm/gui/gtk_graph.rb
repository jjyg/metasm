...
class GraphViewWidget < Gtk::HBox
	...
	def paint
		...
		@curcontext.box.each { |b|
			b.to.each { |tb|
				...
				w.draw_line(gc, srcx, srcy, srcx, srcy+margin)
				w.draw_line(gc, dstx, dsty-margin, dstx, dsty)
				w.draw_line(gc, dstx-margin/2, dsty-margin/2, dstx, dsty)
				w.draw_line(gc, dstx+margin/2, dsty-margin/2, dstx, dsty)
				srcy += margin
				dsty -= margin
				if dsty+margin >= srcy-margin
					w.draw_line(gc, srcx, srcy, dstx, dsty) if srcx != srcy or dstx != dsty
				elsif srcx-b.w/2-margin >= dstx+tb.w/2+margin	# z
					w.draw_line(gc, srcx, srcy, srcx-b.w/2-margin, srcy)
					w.draw_line(gc, srcx-b.w/2-margin, srcy, dstx+tb.w/2+margin, dsty)
					w.draw_line(gc, dstx+tb.w/2+margin, dsty, dstx, dsty)
					...
					w.draw_line(gc, srcx, srcy, x, srcy)
					w.draw_line(gc, x, srcy, x, dsty)
					w.draw_line(gc, x, dsty, dstx, dsty)
				end
			}
		}
		...
		@curcontext.box.each { |b|
			...
			w.draw_layout(gc, b.x-@curcontext.view_x+1, b.y-@curcontext.view_y+1, b.rdtext)
		}
	end
	...
	# rebuild the code flow graph (function graph + each function block graph)
	def gui_update
