...
EOS
	...
	def pehash(digest)
		...
		if @directory['certificate_table']
			off2 = @coff_offset + @header.sizeof(self) + @optheader.sizeof(self) + 8*DIRECTORIES.index('certificate_table')
			off3 = @encoded.length - @directory['certificate_table'][1]
		end
