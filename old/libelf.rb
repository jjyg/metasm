...
class SectionHeader
	...
	def initialize(raw, off)
		array = raw[off .. (off + 10 * 8)].unpack('LLLLLLLLLL')
		@sh_name = array[0]
		@sh_type = array[1]
		@sh_flags = array[2]
		@sh_addr = array[3]
...
class ProgramHeader
	...
	attr_reader :p_type, :p_filesz, :p_offset, :p_vaddr
	def initialize(raw, off)
		array = raw[off .. (off + 8 * 4)].unpack('LLLLLLLL')
		@p_type = array[0]
		@p_offset = array[1]
		@p_vaddr = array[3]
		@p_paddr = array[4]
...
class SymbolTableEntry
	...
	attr_reader :st_name, :st_value, :st_type, :st_bind
	attr_accessor :name
	def initialize(raw, off)
		array = raw[off .. (off + 8 * 3 + 2 * 1 + 2)].unpack('LLLccS')
		@st_name = array[0]
		@st_value = array[1]
		@st_size = array[2]
		@st_info = array[3]
...
class Relocation
	...
	def initialize(raw, off, sym_sh, dest_sh, rela)
		array = raw[off .. (off + 4 * 2)].unpack(rela ? 'LLl' : 'LL')
		@r_offset = array[0]
		@r_info = array[1]
...
class Dynamic
	...
	def initialize(raw, off)
		array = raw[off .. (off + 4 * 2)].unpack('LL')
		@d_tag = array[0]
		@d_val = @d_ptr = array[1]
	end
...
class Header
	...
	attr_reader :e_entry
	def initialize(raw)
		array = raw[0 .. 51].unpack('a16' + 'SSLLLLLSSSSSS')
		@e_ident = array[0]
		@e_type = array[1]
		@e_machine = array[2]
		@e_version = array[3]
		...
		#TODO: handle foreign byte order
		@ei_class = @e_ident[4]
		@ei_data = @e_ident[5]
		...
		if (@e_machine != 3 && @e_machine != 6)
			raise RuntimeError.new("Invalid ELF arch")
		end
...
class ELF
	...
	def find_section(sname)
		@header.sections.select { |e| e.name == sname }[0]
	end
